...
const clients = new Map<number, TokenValidatorClient>(
  SUPPORTED_CHAINS.map((chain) => [
    chain.id,
    createPublicClient({
      chain,
      transport: http(chain.rpcUrls.default.http[0]),
    }) as TokenValidatorClient,
  ]),
);
...
async function validateChainTokens(
  ...
  try {
    ...
    return contractTokens.map((token) => ({
      ...
      chainId,
      issues: [
        `Multicall failed: ${err instanceof Error ? err.message : String(err)}`,
      ],
    }));
  }
  ...
  const contractResults: ValidationResult[] = contractTokens.map((token, i) => {
    ...
    if (!nameRes || !symbolRes || !decimalsRes) {
      issues.push(
        'Incomplete multicall response — expected 3 results per token',
      );
    ...
    if (
      ...
      issues.push(
        'Unable to fetch on-chain data — contract may not exist or not be ERC20',
      );
      return { address: token.address, symbol: token.symbol, chainId, issues };
    }
    ...
    if (token.name !== onChainName) {
      issues.push(
        `Name mismatch: tokenlist="${token.name}", onchain="${onChainName}"`,
      );
    }
    if (token.symbol !== onChainSymbol) {
      issues.push(
        `Symbol mismatch: tokenlist="${token.symbol}", onchain="${onChainSymbol}"`,
      );
    }
    if (token.decimals !== onChainDecimals) {
      issues.push(
        `Decimals mismatch: tokenlist=${token.decimals}, onchain=${onChainDecimals}`,
      );
    }
...
async function validateSchema() {
  ...
  const schema = (await schemaResponse.json()) as AnySchema;
...
async function main() {
  ...
  const chainEntries = [...byChain.entries()].filter(([chainId]) =>
    chainById.has(chainId),
  );
  ...
  const settled = await Promise.allSettled(
    chainEntries.map(([chainId, chainTokens]) => {
      ...
      if (!client) {
        throw new Error(`Missing public client for chainId ${chainId}`);
      }
      return validateChainTokens(chainId, chainTokens, client);
    }),
  );
  ...
  for (let i = 0; i < chainEntries.length; i++) {
    ...
    if (!chain) {
      throw new Error(`Missing configured chain for chainId ${chainId}`);
    }
    ...
    if (outcome.status === 'rejected') {
      console.log(
        `  ❌ Chain validation failed: ${outcome.reason instanceof Error ? outcome.reason.message : String(outcome.reason)}`,
      );
      for (const token of chainTokens) {
        allResults.push({
          address: token.address,
          symbol: token.symbol,
          chainId,
          issues: ['Chain validation failed'],
        });
      }
    ...
    if (dupes.length > 0) {
      console.log(
        `  ⚠️  Duplicate addresses: ${[...new Set(dupes)].join(', ')}`,
      );
    }
    ...
    if (dupeSymbols.length > 0) {
      console.log(
        `  ⚠️  Duplicate symbols: ${[...new Set(dupeSymbols)].join(', ')}`,
      );
    }
  }
  ...
  const tokensWithIssues = allResults.filter(
  ...
  console.log(
    `   Valid tokens:        ${allResults.length - tokensWithIssues - skippedCount}`,
  );
