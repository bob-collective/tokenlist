...
| `underlying` | No | Terminal/root underlying asset ID for wrapped or derivative assets (e.g., `BTC`). This points directly to the final underlying asset, not an intermediate wrapper. Use it only for classification; never derive asset price from it. Use `coingeckoId` for price feeds. Emitted as `extensions.underlying` in generated token lists. |
