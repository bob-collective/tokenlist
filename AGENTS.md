...
- `tsconfig.json` — Composite project covering only the root `*.ts` modules; emits declarations/JS to `dist/` (gitignored, published alongside source files — `main` stays `index.ts`). `scripts/` is excluded and runs via `tsx`, so build scripts are not type-checked.
